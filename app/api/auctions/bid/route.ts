...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!auction || auction.status !== 'ACTIVE') {
      return NextResponse.json({ error: 'Auction not available' }, { status: 400 });
    }
    ...
    const bid = await db.auctionBid.create({
      data: { auctionId, userId: session.user.id, amount },
    });
    await db.auction.update({ where: { id: auctionId }, data: { currentBid: amount, winnerId: session.user.id } });
